...
export default function IngredientManagement() {
    ...
    const fetchIngredients = async () => {
        try {
            ...
            setErrorMessage("Failed to fetch ingredients. Please check your network connection.");
        }
    };
    ...
    const handleSaveIngredient = async () => {
        ...
        if (isNaN(parsedQuantity) || isNaN(parsedThreshold)) {
            alert("Please enter valid numbers for Quantity and Threshold.");
            return;
        }
     ...
     const handleDeleteIngredient = async (id: number) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const errorData = await response.json();
                setErrorMessage(errorData.error || "Failed to delete ingredient.");
            }
        } catch (error) {
    ...
    return (
      <div className="flex h-screen">
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-gray-50">
          <h1 className="text-2xl font-semibold mb-6">Ingredient Management</h1>
          ...
          {ingredients.some(
            ...
            <div className="bg-red-100 text-red-700 p-4 mb-4 rounded-md">
              Warning: Some ingredients are low on stock. Please restock soon.
            </div>
          )}
              ...
              <Card key={ingredient.id} className="bg-white">
                <CardContent className="p-6">
                  ...
                  <div className="space-y-2 text-gray-600">
                    ...
                    <p>Low Stock Threshold: {ingredient.threshold}</p>
                  </div>
                </CardContent>
                <CardFooter className="grid grid-cols-3 gap-2 p-4 bg-gray-50">
                  <Button
                    ...
                    onClick={() => openAddStockModalForIngredient(ingredient)}
                  >
          ...
          {filteredIngredients.length === 0 && (
            <p className="text-center text-gray-500 mt-8">No ingredients found.</p>
          )}
          ...
          <Dialog open={showAddStockModal} onOpenChange={setShowAddStockModal}>
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                <DialogTitle>Add Stock for {selectedIngredient?.name}</DialogTitle>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="addStock" className="text-right">
                    Amount
                  </Label>
                  <Input
                    id="addStock"
